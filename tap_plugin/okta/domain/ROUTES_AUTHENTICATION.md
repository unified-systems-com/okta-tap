...
An identity provider routing rule (IDP_DISCOVERY policy) sends matching sign-ins to an identity provider instead of Okta's own sign-in. Adding a routing rule is how inbound federation is switched on for real users.

## Purpose

Show which sign-ins are sent to an external IdP.
