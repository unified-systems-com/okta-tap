...
A policy evaluates its rules in priority order; the first rule whose conditions match decides. Containment: a rule exists only as part of its policy and retires with it.

## Purpose

Order a policy's rules and retire them with it.
...
- Okta's own policy/rule containment; no graph source models rules separately. Sources: Cartography Okta intel module, `cartography/models/okta/` at commit 030848289203969e19a17bf25e45ff94a527bcef (2026-08-30); BloodHound OpenGraph Okta extension schema (bloodhound.specterops.io/opengraph/extensions/okta/schema, read 2026-09-22); collector SpecterOps/OktaHound release 2.8.2 (2026-04-21).

## Endpoints

- sources: `okta__okta_policy`
- targets: `okta__okta_policy_rule`
- dimensions stamped: none (the edge's dcom value belongs to whatever writes it)
- property `priority` — The rule's position (1 first). Mirrors the rule's own priority field so a traversal can order without a join.
