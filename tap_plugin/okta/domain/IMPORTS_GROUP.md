...
An application masters a group that Okta imports from it (Active Directory, LDAP, an HR system): the group's membership is decided in the application, not in Okta. The source of an APP_GROUP.

## Purpose

Mark groups whose membership is decided outside Okta.
