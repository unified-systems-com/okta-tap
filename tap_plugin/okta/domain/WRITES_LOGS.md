...
A log stream writes the System Log to its destination (an AWS EventBridge partner event source, a Splunk HTTP Event Collector). The target is OPEN (omitted): the destination is another plugin's node, when it is modelled at all. Action and data both flow forward.

## Purpose

Show where the System Log is delivered.
