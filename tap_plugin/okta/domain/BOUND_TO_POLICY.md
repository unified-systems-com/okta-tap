...
An application is bound to the authentication policy (ACCESS_POLICY) Okta evaluates whenever a user signs in to it. Every app has exactly one; many apps may share one.

## Purpose

Show which authentication policy guards each application.
