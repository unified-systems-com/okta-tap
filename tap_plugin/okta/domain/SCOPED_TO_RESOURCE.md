...
A role assignment is limited to a scope: a resource set (custom roles) or target groups and applications (standard roles such as Group Administrator). No scope edge means the assignment covers the whole org, which is the dangerous case.

## Purpose

Say what an admin assignment covers; none means the whole org.
