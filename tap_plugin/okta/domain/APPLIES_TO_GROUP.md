...
A policy (global session, password, enrollment) or a policy rule (authentication policy rules) names a group in its people condition, to include or exclude it.

## Purpose

Show who a policy or rule covers, and who it excludes.
...
- No surveyed graph carries it; it is the join every 'is MFA enforced for X' question needs. Sources: Cartography Okta intel module, `cartography/models/okta/` at commit 030848289203969e19a17bf25e45ff94a527bcef (2026-08-30); BloodHound OpenGraph Okta extension schema (bloodhound.specterops.io/opengraph/extensions/okta/schema, read 2026-09-22); collector SpecterOps/OktaHound release 2.8.2 (2026-04-21).

## Endpoints

- sources: `okta__okta_policy`, `okta__okta_policy_rule`
- targets: `okta__okta_group`
- dimensions stamped: none (the edge's dcom value belongs to whatever writes it)
- property `mode` — Whether the condition includes or excludes the far node. An exclusion is as load-bearing as an inclusion: an excluded group is exactly who the rule does not protect.
