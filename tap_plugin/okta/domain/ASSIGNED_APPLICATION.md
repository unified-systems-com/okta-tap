...
A user or a group is assigned an application: the assignment is what puts the app on a user's dashboard and lets Okta issue an assertion for them. A user's assignment may itself come from a group (scope GROUP).

## Purpose

Answer who can sign in to each application.
...
- BloodHound `Okta_AppAssignment`; Cartography `APPLICATION`. Sources: Cartography Okta intel module, `cartography/models/okta/` at commit 030848289203969e19a17bf25e45ff94a527bcef (2026-08-30); BloodHound OpenGraph Okta extension schema (bloodhound.specterops.io/opengraph/extensions/okta/schema, read 2026-09-22); collector SpecterOps/OktaHound release 2.8.2 (2026-04-21).

## Endpoints

- sources: `okta__okta_user`, `okta__okta_group`
- targets: `okta__okta_application`
- dimensions stamped: none (the edge's dcom value belongs to whatever writes it)
- property `scope` — For a user assignment: USER when assigned directly, GROUP when inherited from a group assignment.
- property `priority` — For a group assignment: its priority when several groups assign the same app with different profiles.
- property `status` — Okta's assignment status (ACTIVE, PROVISIONED, ...).
