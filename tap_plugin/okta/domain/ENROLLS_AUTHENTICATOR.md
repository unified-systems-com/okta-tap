# Enrolls Authenticator

## Blurb

An authenticator enrollment policy (MFA_ENROLL) makes an authenticator required, optional or disallowed for the groups it applies to.

## Purpose

Say which authenticators users are made to enroll.
...
- property `enroll` — Okta's settings.authenticators[].enroll.self.
