...
A user has enrolled an authenticator (Okta's factor enrollment). An edge, not a node: Cartography's OktaUserFactor is this relationship, and everything it carries is a property of the pair.

## Purpose

Record which users enrolled which authenticators.
...
- property `status` — Okta's factor status.
- property `factor_type` — Okta's factorType for the enrollment (push, signed_nonce, webauthn, sms, call, email, token:software:totp, ...).
- property `created_at` — When the factor was enrolled (ISO-8601).
- property `last_verified_at` — When the factor was last used to verify (ISO-8601).
